...
public interface VisitRepository extends JpaRepository<Visit, Integer> {
    ...
    public boolean collidingVisitExist(LocalTime visitStart, LocalTime visitEnd, int DoctorId);
    ...
    public boolean visitExistsForSchedule(int doctorId, LocalTime shiftStart, LocalTime shiftEnd);

    @Query("SELECT CASE WHEN COUNT(v) > 0 THEN true ELSE false END FROM Visit v " +
            "WHERE v.doctor.id = :doctorId  AND " +
            "(v.visitStart < :visitEnd AND v.visitEnd > :visitStart)")
    public boolean visitAlreadyExistsForDoctor(int doctorId, LocalTime visitStart, LocalTime visitEnd);

    @Query("SELECT CASE WHEN COUNT(v) > 0 THEN true ELSE false END FROM Visit v " +
            "WHERE v.consultingRoom.id = :consultingRoomId AND " +
            "(v.visitStart < :visitEnd AND v.visitEnd > :visitStart)")
    public boolean visitAlreadyExistsForConsultingRoom(int consultingRoomId, LocalTime visitStart, LocalTime visitEnd);

    @Query("SELECT CASE WHEN COUNT(v) > 0 THEN true ELSE false END FROM Visit v " +
            "WHERE v.patient.id = :patientId AND " +
            "(v.visitStart < :visitEnd AND v.visitEnd > :visitStart)")
    public boolean visitAlreadyExistsForPatient(int patientId, LocalTime visitStart, LocalTime visitEnd);
